...
@property (strong,nonatomic, nullable) NSDate *lastFailoverTime;
@property (nonatomic) NSTimeInterval connectionRecoveryTime;
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary numberFormatter:(NSNumberFormatter *)numberFormatter dateFormatter:(NSDateFormatter *)dateFormatter
{
    if ((self = [super init])) {
        ...
        self.connectionRecoveryTime = INFINITY;
    }
    return self;
}
...
{
    _connectionRecoveryTime = connectionRecoveryTime;
}

- (void)didFailAtTime:(NSDate *)time
{
    _lastFailoverTime = time;
}

- (BOOL)isEnabledForSelectionAtTime:(NSDate *)time
{
    if (!_lastFailoverTime) {
        return YES;
    }
    
    return time.timeIntervalSince1970 - _lastFailoverTime.timeIntervalSince1970 >= _connectionRecoveryTime;
}

...
#import "BBCMediaItem+Internal.h"
...
{
    for (BBCMediaItem *item in _items) {
        if ([configuring respondsToSelector:@selector(connectionRecoveryTime)]) {
            [item setConnectionRecoveryTime:[configuring connectionRecoveryTime]];
        } else {
            NSTimeInterval defaultRecoveryTime = 2.0 * 60.0;
            [item setConnectionRecoveryTime:defaultRecoveryTime];
        }
    }
}

...
-(BOOL)playheadIsBeforeLiveWindow:(NSTimeInterval) startOfSeekableRange {
    // Values are rounded to hundreths of a second
    // to work around  a bug when seeking a webcast.
    // See BUG_MOBILE_7473 test class.
    int timescale = 100;
    
    CMTime playheadAbsoluteTime = CMTimeMakeWithSeconds([_playerItem.currentDate timeIntervalSince1970], timescale*10);
    playheadAbsoluteTime = CMTimeConvertScale(playheadAbsoluteTime, timescale, kCMTimeRoundingMethod_Default);
    CMTime seekableRangeStart = CMTimeMakeWithSeconds(startOfSeekableRange, timescale*10);
    seekableRangeStart = CMTimeConvertScale(seekableRangeStart, timescale, kCMTimeRoundingMethod_Default);
    
    // (== -1) equivalent to playheadAbsoluteTime < seekableRangeStart
    return CMTimeCompare(playheadAbsoluteTime, seekableRangeStart) == -1;
}

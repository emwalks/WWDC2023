...
- (void)timeChanged:(BBCSMPTime*)time
{
    ...
    [_observerManager notifyObserversForProtocol:@protocol(BBCSMPAVStatisticsConsumer)
                                       withBlock:^(id<BBCSMPAVStatisticsConsumer> avConsumer) {
        if ([avConsumer respondsToSelector:@selector(progress:)]) {
            BBCSMPMediaProgress* mediaProgress = [[BBCSMPMediaProgress alloc] initWithStartPosition: [[BBCSMPMediaPosition alloc] initWithSeconds:0]
                                                                                        endPosition: [[BBCSMPMediaPosition alloc] initWithSeconds:0]
                                                                                      mediaPosition: [[BBCSMPMediaPosition alloc] initWithSeconds:time.seconds]];
            [avConsumer progress:mediaProgress];
        }
    }];
}

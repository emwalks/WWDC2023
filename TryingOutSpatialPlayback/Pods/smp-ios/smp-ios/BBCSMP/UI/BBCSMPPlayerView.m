...
@property (nonatomic, copy) void (^airplayDeactivatedAction)(void);
@property (nonatomic, strong) BBCSMPStockUIBackgroundManager *stockUIBackgroundManager;
@end
...
@implementation BBCSMPPlayerView {
    ...
    id<BBCSMPVideoTrackSubscriber> _Nullable _videoTrackSubscriber;
}
         ...
         videoTrackSubscriber:(id<BBCSMPVideoTrackSubscriber>)videoTrackSubscriber
{
    if ((self = [super initWithFrame:frame])) {
        ...
        _stockUIBackgroundManager = [[BBCSMPStockUIBackgroundManager alloc] initWithSmpControllable:player
                                                                                      smpObservable:player
                                                                            backgroundStateProvider:backgroundStateProvider];
    }
    return self;
}
...
- (void)initialiseUserInterface
{
    ...
    self.subtitleView = [[[[BBCSMPSubtitleViewBuilder alloc] initWithPlayer:_player] withConfiguration:_configuration] createSubtitleView];
...
- (void)attachPlayer:(id<BBCSMP, BBCSMPExperimental>)player
{
...
- (void)pauseWhenAirplayDeactivates {
    __weak typeof(self) weakSelf = self;
    self.airplayDeactivatedAction = ^void() {
        [weakSelf.player pause];
        weakSelf.airplayDeactivatedAction = nil;
    };
}
...
- (void)airplayActivationChanged:(NSNumber*)active {
    if (!active.boolValue) {
        [self.airplayDeactivatedAction invoke];
    }
    else {
        [self pauseWhenAirplayDeactivates];
    }
}
...
#pragma mark BBCSMPVideoTrackSubscriber

- (void)videoTrackAvailable:(id<BBCSMPVideoTrack>)videoTrack
{
    [_videoTrackSubscriber videoTrackAvailable:videoTrack];
}
